...
resource "aws_vpc_endpoint" "secretsmanager" {
  ...
  tags = merge(var.common_tags, { Name = "${var.project_name}-consul-secretsmanager" })
}
...
resource "aws_vpc_endpoint" "ec2" {
  ...
  tags = merge(var.common_tags, { Name = "${var.project_name}-consul-ec2" })
}
...
resource "aws_vpc_endpoint" "ssm" {
  ...
  tags = merge(var.common_tags, { Name = "${var.project_name}-consul-ssm" })
}
...
resource "aws_vpc_endpoint" "s3" {
  ...
  tags = merge(var.common_tags, { Name = "${var.project_name}-consul-s3" })
}

# Security Groups

resource "aws_security_group" "bastion" {
  name_prefix = "${var.project_name}-consul-bastion-"
...
resource "aws_security_group" "vpc_endpoints" {
  ...
  name_prefix = "${var.project_name}-consul-vpc-endpoints-"
  description = "Security group for VPC endpoints"
  vpc_id      = module.vpc[0].vpc_id

  tags = merge(var.common_tags, { Name = "${var.project_name}-consul-vpc-endpoints" })
