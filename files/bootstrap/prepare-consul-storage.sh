#!/bin/sh
# prepare-consul-storage.sh
#
# Waits for the EBS volumes for Consul Raft data and audit logs to appear as
...
# sets ownership on the mount points so the consul user can read/write.
# Runs on every node before consul.service starts.
...
readonly CONSUL_AUDIT_LOG_DIR="/var/log/consul"
readonly CONSUL_RAFT_DATA_DIR="/var/opt/consul/data"
...
set_ownership_and_permissions() (
  mountpoint -q "${CONSUL_RAFT_DATA_DIR}" ||
    {
      log_error "${CONSUL_RAFT_DATA_DIR} not mounted; refusing to chown underlying directory"
      return 1
    }
  mountpoint -q "${CONSUL_AUDIT_LOG_DIR}" ||
    {
      log_error "${CONSUL_AUDIT_LOG_DIR} not mounted; refusing to chown underlying directory"
      return 1
    }

  chown consul:consul "${CONSUL_RAFT_DATA_DIR}"
  chmod 700 "${CONSUL_RAFT_DATA_DIR}"

  chown consul:consul "${CONSUL_AUDIT_LOG_DIR}"
  chmod 750 "${CONSUL_AUDIT_LOG_DIR}"
)

main() {
  # Consul Raft Data
  consul_raft_data_nvme_block_device_path="$(resolve_ebs_nvme_block_device_path "${CONSUL_RAFT_DATA_EBS_ATTACHMENT_NAME}")"
  prepare_ebs_volume "${consul_raft_data_nvme_block_device_path}" "${CONSUL_RAFT_DATA_DIR}" "consul-raft"

  # Consul Audit Log
  consul_audit_log_nvme_block_device_path="$(resolve_ebs_nvme_block_device_path "${CONSUL_AUDIT_LOG_EBS_ATTACHMENT_NAME}")"
  prepare_ebs_volume "${consul_audit_log_nvme_block_device_path}" "${CONSUL_AUDIT_LOG_DIR}" "consul-audit"
