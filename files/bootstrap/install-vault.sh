...
# install-vault.sh
#
# Downloads, GPG-verifies, SHA256-verifies, and installs the Vault community
# edition binary at /usr/local/bin/vault. Only agent mode is used, to issue
# TLS certificates and render the gossip encryption key from the external
# Vault cluster. Runs on every node before the cluster bootstrap.
...
fetch_vault_release_and_signing_key() (
  ...
  log_info "Downloading Vault ${VAULT_AGENT_VERSION}"
  curl --fail --silent --show-error --location \
    --output "${TMPDIR_SESSION}/${vault_release_filename}" \
    "https://releases.hashicorp.com/vault/${VAULT_AGENT_VERSION}/${vault_release_filename}"

  log_info "Downloading Vault ${VAULT_AGENT_VERSION} SHA256SUMS file"
  curl --fail --silent --show-error --location \
    --output "${TMPDIR_SESSION}/${vault_release_sha256sums_filename}" \
    "https://releases.hashicorp.com/vault/${VAULT_AGENT_VERSION}/${vault_release_sha256sums_filename}"

  log_info "Downloading Vault ${VAULT_AGENT_VERSION} SHA256SUMS signature file"
  curl --fail --silent --show-error --location \
    --output "${TMPDIR_SESSION}/${vault_release_sha256sums_filename}.sig" \
    "https://releases.hashicorp.com/vault/${VAULT_AGENT_VERSION}/${vault_release_sha256sums_filename}.sig"
...
install_vault() (
  ...
  log_info "Installing Vault ${VAULT_AGENT_VERSION}"
...
main() {
  vault_release_filename="vault_${VAULT_AGENT_VERSION}_linux_$(detect_system_architecture).zip"
  vault_release_sha256sums_filename="vault_${VAULT_AGENT_VERSION}_SHA256SUMS"
