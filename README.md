...
| <a name="input_bootstrap"></a> [bootstrap](#input\_bootstrap) | AWS resources used only during the Consul bootstrap ceremony. SSM<br/>parameters hold non-sensitive coordination state shared between nodes. | <pre>object({<br/>    ssm_parameter = optional(object({<br/>      consul_cluster_state_name = optional(string, "/consul-enterprise/bootstrap/cluster/state")<br/>      instance_id_name          = optional(string, "/consul-enterprise/bootstrap/instance/id")<br/>    }), {})<br/>  })</pre> | `{}` | no |
